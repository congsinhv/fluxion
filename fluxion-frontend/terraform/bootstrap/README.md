# fluxion-frontend — Terraform State Bootstrap

## Purpose

One-time creation of the S3 bucket that stores all Terraform remote state for the
`fluxion-frontend` sub-repo. The bootstrap itself runs with **local state** (no remote
...
- `terraform/bootstrap/terraform.tfstate` (local) must be kept outside version control —
  add it to `.gitignore`.
- Bucket names are globally unique in AWS. If `fluxion-frontend-tfstate` is already
  taken, append a short suffix, e.g. `fluxion-frontend-synh`.
