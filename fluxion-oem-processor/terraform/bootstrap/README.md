# fluxion-oem-processor — Terraform State Bootstrap

## Purpose

One-time creation of the S3 bucket that stores all Terraform remote state for the
`fluxion-oem-processor` sub-repo. The bootstrap itself runs with **local state** (no remote
...
3. Create the S3 bucket:
   ```
   terraform apply -var=resource_name_prefix=fluxion-oem-processor
   ```
...
- `terraform/bootstrap/terraform.tfstate` (local) must be kept outside version control —
  add it to `.gitignore`.
- Bucket names are globally unique in AWS. If `fluxion-oem-processor-tfstate` is already
  taken, append a short suffix, e.g. `fluxion-oem-processor-synh`.
